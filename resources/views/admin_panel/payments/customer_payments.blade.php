...
<body>
        ...
        <div class="body-wrapper">
            <div class="bodywrapper__inner">
                ...
                <div class="container bg-white p-4 shadow rounded">
                    ...
                    <form action="{{ route('customer.payment.store') }}" method="POST" data-shortcut="r">
    ...
    <script>
(function () {
    // Pehle shortcut-flag wali form dhundo; warna pehli POST form le lo
    const form = document.querySelector('form[data-shortcut="r"]') 
              || document.querySelector('form[method="POST"]');
    if (!form) return;

    window.addEventListener('keydown', function (e) {
        const key = (e.key || '').toLowerCase();
        // Mac (⌘) bhi support & repeat press ignore
        if ((e.ctrlKey || e.metaKey) && key === 'r' && !e.repeat) {
            e.preventDefault(); // Browser ka reload block
            if (form.reportValidity && !form.reportValidity()) return; // HTML5 validation
